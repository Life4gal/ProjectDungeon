...
#include <components/entity/terrain.hpp>
	...
	namespace
	{
		...
		) noexcept -> entt::entity
		{
			...
			// physics
			{
				...
				if constexpr (High)
				{
					shape_def.filter.categoryBits = static_cast<std::uint64_t>(EntityType::HIGH_TERRAIN);
					shape_def.filter.maskBits = CollisionMask::high_terrain;
				}
				else
				{
					shape_def.filter.categoryBits = static_cast<std::uint64_t>(EntityType::LOW_TERRAIN);
					shape_def.filter.maskBits = CollisionMask::low_terrain;
				}
