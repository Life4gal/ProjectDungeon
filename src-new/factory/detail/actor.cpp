...
namespace pd::factory::detail
{
	...
	auto attach(entt::registry& registry, const entt::entity entity, const blueprint::Actor& actor, const blueprint::Size size, const blueprint::Position origin) noexcept -> void
	{
		// 生命值条|魔法值条与精灵的最小偏移
		const auto base_offset_x = origin.x;
		const auto base_offset_y = origin.y + 5.f;

		// 生命值条|魔法值条的宽度&高度
		const auto bar_width = size.width * 1.2f;
		const auto bar_height = size.height / 4;

		// 偏移1(上面那根)
		const auto offset_x_1 = -(base_offset_x + (bar_width - size.width) / 2);
	...
	{
		attach(registry, entity, actor, sprite.size, sprite.origin);
	}
	...
	{
		attach(registry, entity, actor, sprite_animation.size, sprite_animation.origin);
	}
}
